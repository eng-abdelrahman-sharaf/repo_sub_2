...
def run(settings):
    ...
    # ----------------------------
    # NEW: Fine-tune from a given checkpoint path (student + adaptive_net)
    # ----------------------------
    resume_path = getattr(cfg.TRAIN, "RESUME_PATH", "")
    resume_path = "" if resume_path is None else str(resume_path)

    if resume_path.strip():
        resume_path = os.path.expanduser(resume_path)
        if settings.local_rank in [-1, 0]:
            print(f"[Fine-tune] TRAIN.RESUME_PATH is set. Loading student checkpoint from: {resume_path}")
        if not os.path.isfile(resume_path):
            raise FileNotFoundError(
                f"TRAIN.RESUME_PATH does not exist or is not a file: {resume_path}\n"
                f"Please set TRAIN.RESUME_PATH to a valid checkpoint file (e.g., uetrack_base.tar)."
            )
        # This uses strict=False internally and expects keys 'net' and 'adaptive_net' in the checkpoint dict.
        trainer.load_state_dict(resume_path)
        load_latest = False  # do NOT also auto-resume from save_dir
    else:
        load_latest = True   # keep old behavior
