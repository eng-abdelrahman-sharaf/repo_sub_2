...
class UETrack(nn.Module):
    ...
    def __init__(self, text_encoder, encoder, decoder, task_decoder,
        ...
        if self.text_encoder is not None:
            self.interface_text_proj = nn.Linear(self.text_encoder.textencoder_dim,
...
def build_uetrack(cfg):
    ...
    #load teacher model
    teacher_path = getattr(cfg.TRAIN, "TEACHER_PATH", "")
    teacher_path = "" if teacher_path is None else str(teacher_path)

    if teacher_path.strip():
        if not os.path.isfile(teacher_path):
            raise FileNotFoundError(f"TRAIN.TEACHER_PATH does not exist: {teacher_path}")

        teacher_checkpoints = torch.load(teacher_path, map_location="cpu")
        state_dict = teacher_checkpoints["net"]  # <- your existing line
        teacher_model.load_state_dict(state_dict, strict=True)  # <- keep strict as-is
        for p in teacher_model.parameters():
            p.requires_grad = False
    else:
        # IMPORTANT: do not call load_state_dict() here
        # Keep teacher_model as randomly initialized (or same init as student)
        try:
            local_rank = getattr(cfg, "LOCAL_RANK", -1)
        except Exception:
            local_rank = -1
        if local_rank in [-1, 0]:
            print("[UETrack] TRAIN.TEACHER_PATH is empty. Skipping teacher checkpoint load (fine-tuning without distillation).")
